...
@task
def v_dump_one_graph_nyc(sub_path, timestamp, cmd, host, user):
...
@task
def v_dump_one_graph_aws(sub_path, timestamp, cmd, host, user, keyfile):
    print "\nCreating backup on user:%s, host:%s"%(user,host)
    print "In virt_dump_one_graph..."
    with settings(host_string="%s@%s"%(user, host)):
...
@task        
def v_backup_to_local(user, host, srcdir, dir_backup):
...
@task    
def v_backup_to_aws(user, host, dir_backup, keyfile, dstpath):
    env.key_filename = keyfile
