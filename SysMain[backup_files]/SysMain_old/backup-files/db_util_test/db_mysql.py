...
try:
    ...
    print("Error encountered during import of callSubprocess from ttLib.ttLx. Exiting!%s" %str(sys.exc_info()))
...
try:
    ...
    print("Error encountered during import of findSourceSuffix from ttLib.ttAWS. Exiting! %s" %str(sys.exc_info()))
    #sys.exit("Logfile location: %s" %swapTimeStampDirPath) 
...
def mongo_operations(operation, service, s_host, t_host, my_logger):
    ...
    mongoclone(s_host, t_host, my_logger)
    #my_logger.info(clone_OP)
    mongodump_source("afterUpdate-source", s_host, my_logger)
    my_logger.info("Back in mongo_operations. Backup after update successful for source.")
    mongodump_target("afterUpdate-target", t_host, my_logger)
...
def mongodump_target(suffix, t_host, my_logger):
    my_logger.info("In mongodump()...")
    try:
        filename="mongodb-" + suffix 
        filepath=os.path.join(swapTimeStampDirPath,filename)
...
def mongoclone(s_host, t_host, my_logger):
...
def validate_cert_loc():
    try:
        ...
        if (os.access(path_ssl, os.R_OK)):
            ...
            print("Validate SSL failed. Exiting! %s" %str(sys.exc_info()))
            sys.exit("Program terminated on error. Logfile location: %s" %swapTimeStampDirPath)
    except:
        print("Certificate location cannot be validated. Error Encountered: %s. Exiting!" %str(sys.exc_info()))
        sys.exit("Program terminated on error. Logfile location: %s" %swapTimeStampDirPath)
    return path_ssl
...
def mysql_operations(operation, service, aws_cert, source_hostname, target_hostname, my_logger):
    my_logger.info("In mysql_Operations...")
    #print("Source Host: %s " %mysql_source_host)
    #print("Target Host: %s " %mysql_target_host)
    import time
    timestamp= time.strftime("%Y%m%d-%H%M%S")
    dump_source_before=mysqldump_source("source-beforeUpdate", aws_cert, source_hostname, my_logger)
    my_logger.info(dump_source_before)
    
    dump_target_before=mysqldump_target("target-beforeUpdate", aws_cert, target_hostname, my_logger)
    my_logger.info(dump_target_before)
    
    snapshot_source_before= takesnapshot(source_hostname,my_logger,'source','before-copy',timestamp)
    log_str=str("Started clone: "+snapshot_source_before)
    my_logger.info(log_str)
    
    if "aws.3top.com" in target_hostname:
        snapshot_target_before=takesnapshot(target_hostname,my_logger,'target','before-copy',timestamp)
        log_str=str("Started clone: "+snapshot_target_before)
        my_logger.info(log_str)
    
    statusCheck=snapStatusCheck(snapshot_source_before,snapshot_target_before)
    ...
    clone_OP=awsmysqlclone(aws_cert, source_hostname, target_hostname, my_logger)
    my_logger.info(clone_OP)
    
    dump_source_after=mysqldump_source("source-afterUpdate", aws_cert, source_hostname, my_logger)
    my_logger.info(dump_source_after)
    
    dump_target_after=mysqldump_target("target-afterUpdate", aws_cert, target_hostname, my_logger)
    my_logger.info(dump_target_after)
    
    snapshot_source_after=takesnapshot(source_hostname,my_logger,'source', 'after-copy',timestamp)
    log_str=str("Started clone: "+snapshot_source_after)
    my_logger.info(log_str)
    
    if "aws.3top.com" in target_hostname:
        snapshot_target_after=takesnapshot(target_hostname,my_logger,'target', 'after-copy',timestamp)
        log_str=str("Started clone: "+snapshot_target_after)
        my_logger.info(log_str)
        
    statusCheck=snapStatusCheck(snapshot_source_after,snapshot_target_after)
...
def mysqldump_source(suffix, aws_cert, source_hostname, my_logger):
    try:
...
def takesnapshot(host, my_logger,suffix1, suffix2,timestamp):
    import boto.rds2
    Region = 'us-east-1'
    #dict_Dbinstance={}
    print"started"
...
def snapStatusCheck(snapshotId1, snapshotId2):
    import time
    import boto.rds2
    Region="us-east-1"
    conn = boto.rds2.connect_to_region(Region)
