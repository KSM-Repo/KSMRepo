...
domain_name = input("\nAvailable domains: \n1. www.3top.com \n2. www.3top.test \nChoose the domain to run the script: ")
if domain_name == 1:
    s3_config_folder = "nyc-sys"
elif domain_name == 2:
    s3_config_folder = "nyc-sys-dev"
try:
...
print("Logfile Location: %s"%log_file)
from ttLib.ttAWS import selectEnviron, auth_func, dnsEc2ToDict, dnsBeanstalkToDict, dnsRdsToDict, \
    route53Records, find_rrtype, cli53_export, del_cname, dnsUpdate, zoneR53
from ttLib.ttWin import validateWin, findWinZones, enumRecords, deleteRecords, addRecords, winDnsBackup
...
def main():
    ...
    findWinZones(zone_NYC_long, host_windns, host_winsys_password, host_winsys_username, host_winsys)
    """
    #This module finds the Zone ID (Z12...) for the given Zone Name (say, 3top.com)
    """

    my_logger.debug("In dnsOps...calling zoneR53 for collecting Zone ID information...")
    ...
    if len(sys.argv) == 3:
        if sys.argv[1] == "-p":
            ...
            sys.exit("The parameter passed cannot be identified. Use \"-p <active_env_name>\" in maintenance mode (to set production environment)")
    elif (len(sys.argv) == 1) and (sys.argv[0] == "dnsOps.py"):
        m_arg = "NA"
    else:
        sys.exit("Format mismatch. Run the program as \"python dnsOps.py\" or \"python dnsOps.py -p <active_env_name>\" in maintenance mode (to set production environment)\". EXITING!")
    ...
    my_logger.debug("In dnsOps...calling dnsEc2ToDict for collecting DNS records from EC2...")
    (dictNycTemp, dictAwsTemp) = dnsEc2ToDict(REGION_NAME, ENV_NAME, zone_AWS_long)
    ...
    """
    my_logger.debug("In dnsOps...calling dnsBeanstalkToDict for collecting DNS records from BEANSTALK...")
    (dictNycTemp, dictAwsTemp) = dnsBeanstalkToDict(REGION_NAME, ENV_NAME, zone_AWS_long)
    """
    ...
    #fp-rds-1
    """
    my_logger.debug("In dnsOps...calling dnsRdsToDict for collecting DNS records from RDS...")
    (dictNycTemp, dictAwsTemp) = dnsRdsToDict(REGION_NAME, ENV_NAME, zone_AWS_long)
    """
   # Append the DNS records collected from RDS to the global dictionaries
    ...
    for k,v in dict_NYC.items():
        if ENV_NAME in k:
            (outWinEnum, errWinEnum, currRecWinEnum) = enumRecords(k, host_windns, zone_NYC_long, host_winsys_password, host_winsys_username, host_winsys)
    ...
    if confirm_backup == 'y' or confirm_backup == 'Y':
        ...
        print("In dnsOps...calling cli53_export for backing up current AWS DNS Data...")
        my_logger.debug("In dnsOps...calling cli53_export for backing up current AWS DNS Data...")
        ...
        try:
            ...
            out_aws_update = dnsUpdate(ENV_NAME, zone_Id, dict_add_aws_record, zone_AWS_long)
        ...
        try:
            ...
            winDnsBackup("beforeUpdate", host_windns, zone_NYC_long, host_winsys_password, host_winsys_username, host_winsys)
        ...
        try:
            ...
            (cmd, out_del_rec) = deleteRecords(dictRecDelete, host_windns, zone_NYC_long, host_winsys_password, host_winsys_username, host_winsys)
            cmd = str(cmd)
        ...
        try:
            my_logger.info( "In dnsOps...calling addRecords for adding records...")
            (cmd, out_add_rec) = addRecords(dictRecAdd, host_windns, zone_NYC_long, host_winsys_password, host_winsys_username, host_winsys)
            cmd = str(cmd)
        ...
        try:
            my_logger.info( "In dnsOps...calling winDnsBackup for completing post update backup...")
            winDnsBackup("afterUpdate", host_windns, zone_NYC_long, host_winsys_password, host_winsys_username, host_winsys)
        ...
        my_logger.info( "In dnsOps...calling cli53_export for backing up current AWS DNS Data...")
        ...
        print("Confirmation not received. Received choice for update: %s." % confirm_backup)
